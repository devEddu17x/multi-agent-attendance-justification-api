...
export class JustificationService {
  ...
  async persist(
    ...
    this.logger.log(
      `[PERSIST] Starting persist for studentId=${state.studentId}, decision=${JSON.stringify(decision)}`,
    );
    ...
    this.logger.log(
      `[PERSIST] Student found: id=${student.id}, parentId=${student.parentId}`,
    );
    ...
    this.logger.log(`[PERSIST] hasExtractedData=${!!state.extractedData}`);

    if (!dates || dates.length === 0) {
      this.logger.warn(
        '[PERSIST] No absence dates found, updating session only',
      );
      await this.updateSessionOnly(state.sessionId, decision);
      return;
    }
    ...
    if (allRecords.length === 0 && dates && dates.length > 0) {
      this.logger.log(
        `[PERSIST] No records found, creating pre-absences for dates: ${JSON.stringify(dates)}`,
      );
      await this.attendanceService.createPreAbsences(state.studentId, dates);

      // Fetch all records again (including newly created ones)
      const newRecords =
        await this.attendanceService.getRecordsByStudentAndDates(
          state.studentId,
          dates,
        );
      this.logger.log(
        `[PERSIST] After pre-absences, found ${newRecords.length} records`,
      );

      if (newRecords.length === 0) {
        this.logger.warn(
          '[PERSIST] No records found after pre-absences creation, updating session only',
        );
        await this.updateSessionOnly(state.sessionId, decision);
        return;
      }
  ...
  private async updateSessionOnly(
    sessionId: string,
    decision: TransactionalDecision,
  ): Promise<void> {
    await this.sessionManager.updateSessionStatus(sessionId, {
  ...
  private async processRecords(
    ...
    this.logger.log(
      `[PERSIST] ${pendingRecords.length} records pending justification (already excused: ${records.length - pendingRecords.length})`,
    );

    // Check if there's an existing justification for these records
    const existingJustificationId = records.find(
      (r) => r.justificationId,
    )?.justificationId;
    let existingJustification: AttendanceJustificationEntity | null = null;
    if (existingJustificationId) {
      existingJustification = await this.attendanceService.getJustificationById(
        existingJustificationId,
      );
    }
    ...
    if (
      ...
      this.logger.log(
        `[PERSIST] Updating existing justification ${existingJustificationId} with new evidence`,
      );
      await this.attendanceService.updateJustification(
        existingJustificationId,
        {
      ...
      this.logger.log(
        `[PERSIST] Creating justification with attendanceIds: ${pendingRecords.map((r) => r.id).join(', ')}`,
      );
      ...
      this.logger.log(
        '[PERSIST] All records already excused, skipping justification creation',
      );
    }
  ...
  private extractDates(
    ...
    const dates = (extractedData.absenceDates ??
      extractedData.fechasAusencia) as string[] | undefined;
    return Array.isArray(dates) && dates.length > 0 ? dates : undefined;
  }
  ...
  private extractReason(extractedData?: Record<string, unknown>): string {
    ...
    return (extractedData.reasonCategory ??
      extractedData.motivoAusencia ??
  ...
  private buildEvidences(
    ...
    return attachments
      ...
      .map((a, index) => ({
        key: a.metadata?.key ?? `attachment_${index}`,
        type: a.metadata?.mediaType ?? a.type,
        name: a.metadata?.name ?? `attachment_${index + 1}`,
      }));
  }
